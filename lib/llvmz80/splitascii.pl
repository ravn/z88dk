...
# splits any `.ascii "..."` line whose EMITTED text exceeds a safe threshold
# into several consecutive `.ascii "..."` lines, splitting only on real byte
# boundaries (never mid-escape), so copt sees several short lines instead of
# one long one and its ordinary `.ascii %1 -> DEFM %1` rule applies to each
# piece unchanged.  A label on the original line is kept on the first piece
# only; splitting mid-array does not move where the label points (it is at
# the START of the data, which is still the first emitted byte).
#
