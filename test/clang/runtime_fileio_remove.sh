#!/bin/sh
# Oracle-based runtime test: remove() deletes file; subsequent fopen returns NULL.
# See runtime_fileio_remove.c for the full description.
#
# GREEN (classic): sccz80 and llvmz80 both produce "remove[ok]"
...
SRC="$DIR/runtime_fileio_remove.c"
...
if [ "$ACTUAL" = "$EXPECTED" ]; then
    echo "PASS: remove() then fopen returns NULL (got [$ACTUAL])"
else
    echo "XFAIL: remove output differs from sccz80 oracle. got=[$ACTUAL] want=[$EXPECTED]"
fi
