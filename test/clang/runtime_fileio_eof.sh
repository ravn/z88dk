#!/bin/sh
# Oracle-based runtime test: fgets loop to feof, line counting.
# See runtime_fileio_eof.c for the full description.
#
# GREEN (classic): sccz80 and llvmz80 both produce "eof[3][line-c]"
...
if [ "$ACTUAL" = "$EXPECTED" ]; then
    echo "PASS: fgets/feof line loop (got [$ACTUAL])"
else
    echo "XFAIL: fgets/feof output differs from sccz80 oracle. got=[$ACTUAL] want=[$EXPECTED]"
fi
