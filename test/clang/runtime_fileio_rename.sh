#!/bin/sh
# Oracle-based runtime test: rename() then fopen under new name.
# See runtime_fileio_rename.c for the full description.
#
# GREEN (classic): sccz80 and llvmz80 both produce "rename[renamed]"
...
SRC="$DIR/runtime_fileio_rename.c"
...
if [ "$ACTUAL" = "$EXPECTED" ]; then
    echo "PASS: rename() then fopen new name (got [$ACTUAL])"
else
    echo "XFAIL: rename output differs from sccz80 oracle. got=[$ACTUAL] want=[$EXPECTED]"
fi
