#!/bin/sh
# Oracle-based runtime test: fopen non-existent file returns NULL.
# See runtime_fileio_null.c for the full description.
#
# GREEN (classic): sccz80 and llvmz80 both produce "null[ok]"
...
if [ "$ACTUAL" = "$EXPECTED" ]; then
    echo "PASS: fopen missing file returns NULL (got [$ACTUAL])"
else
    echo "XFAIL: fopen-NULL output differs from sccz80 oracle. got=[$ACTUAL] want=[$EXPECTED]"
fi
