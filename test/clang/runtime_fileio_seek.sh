#!/bin/sh
# Oracle-based runtime test: fseek / ftell.
# See runtime_fileio_seek.c for the full description.
#
# GREEN (classic): sccz80 and llvmz80 both produce "seek[record-1]"
...
if [ "$ACTUAL" = "$EXPECTED" ]; then
    echo "PASS: fseek/ftell random access (got [$ACTUAL])"
else
    echo "XFAIL: fseek/ftell output differs from sccz80 oracle. got=[$ACTUAL] want=[$EXPECTED]"
fi
